		...
		.then((json) => {
			...
			if ("error" in json) {
				...
				json.forEach((job) => {
					...
					// Edit button
					+ "<span onclick=requestRenameJob('" + job.uid + "')><i class=\"clickable-icon icon icon bi-pencil\"></i></span>"
					// Delete Button
					+ "<span onclick=requestDeleteJob('" + job.uid + "')><i class=\"clickable-icon icon icon bi-trash\"></i></span>"
					+ "</h2>"
					// UID
					+ "<div>UID: " + job.uid + "</div>"
					// Status
...
// 					+ "&api_url=" + apiUrlNoHttp
					+ " target=_blank rel=\"noopener noreferrer\"><i class=\"icon just-icon bi-caret-right\"></i>View</a>"
					// Kill Button
					+ "<a class=\"button-small-error " + addlClass + "\" onclick='killJob(\"" + job.uid + "\")' id=kill-job-" + job.uid + "><i class=\"icon just-icon icon bi-stop-circle\"></i>Kill</a>"
					// Hidden logs
